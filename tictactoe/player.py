...
class Player:
    ...
    # we want all players to get theur mext move given a game
...
class RandomComputerPlayer(Player):
    ...
    def get_move(self, game):
        # get a random valid spot for pur next move
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves()) # randomly chooose one
        else:
    ...
    def minimax(self, state, player):
        ...
        # initialize some divtionaries
        if player == max_player:
            best = {'position': None, 'score': -math.inf} # each score should maxnimize (be larger)
        else:
        ...
        for possible_move in state.available_moves():
            ...
            # step 4: update the dictinaries if neccessary
            if player == max_player:
